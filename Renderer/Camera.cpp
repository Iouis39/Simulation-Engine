...
Camera::Camera(simd::float3 position, simd::float3 direction, simd::float3 up, float fov,
...
CameraUniforms& Camera::uniforms() {
    return m_uniforms;
}

void Camera::update(float deltaTime) {
    m_position += m_velocity * deltaTime * m_speed;
//    m_direction += m_velocity * deltaTime * m_speed;
    updateUniforms();
}
